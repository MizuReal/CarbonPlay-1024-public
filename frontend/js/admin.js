      ...
      try {
        ...
        const rows = data.map(p => `
          <tr>
            ...
            <td class="text-right">
              <button class="btn btn-ghost btn-xs" data-action="open-modal" data-modal="modal-profiles" data-mode="edit" data-id="${p.id}"><i class="fa-solid fa-pen"></i></button>
            </td>
          </tr>`);
        renderRows(tbody, rows);
      } catch (e) {
      ...
      try {
        ...
        const rows = data.map(s => `
          <tr>
            ...
            <td>${Number(s.vs_baseline || 0).toFixed(2)}</td>
